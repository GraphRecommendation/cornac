...
def get_best(path, method, default_parameters):
    method_name = method
    ...
    while improved:
        ...
        for phase_parameters in method_dict['phases']:
            ...
            if len(optimal_parameters) > 0:
                b_param = get_best(path, method, default_parameters)
                if b_param is not None:
                    for k, v in b_param.items():
                        if k in optimal_parameters:
                            optimal_parameters[k] = v
                else:
                    print(f'Warning: could not find optimal parameters; using default.')
