...
class Model(nn.Module):
	...
	def inference(self, review_graphs, node_review_graph):
		review_embs = torch.zeros((max(review_graphs)+1, self.review_conv.out_dim))
		...
		review_dataloader = dgl.dataloading.GraphDataLoader(review_dataset, batch_size=1024, shuffle=False,
		...
		self.inf_emb = torch.zeros((torch.max(indices['node'])+1, self.review_agg._out_feats))
