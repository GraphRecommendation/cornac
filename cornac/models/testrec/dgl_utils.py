...
class HearBlockSampler(dgl.dataloading.NeighborSampler):
    def __init__(self, node_review_graph, review_graphs,  aggregator,
                 ntypes, ntype_ranges,
        ...
        self.ntypes = ntypes
        self.ntype_ranges = ntype_ranges
    ...
    def sample(self, g, seed_nodes, exclude_eids=None, pair_graph=None):
        ...
        node_filter = lambda t: (seed_nodes >= self.ntype_ranges[t][0]) * (seed_nodes < self.ntype_ranges[t][1])
        seed_nodes = {nt: seed_nodes[mask] for nt in self.ntypes if torch.any(mask := node_filter(nt))}
        ...
        for nt in self.ntypes:
            nids = seed_nodes[nt]
            _, _, _blocks = super().sample(self.node_review_graph, {'node': nids}, exclude_eids)
            block = _blocks[0]
