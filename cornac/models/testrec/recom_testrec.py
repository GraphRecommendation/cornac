...
class TestRec(Recommender):
    def __init__(self, name='TestRec', use_cuda=False, use_uva=False,
                 batch_size=128,
                 num_workers=0,
                 num_epochs=10,
                 early_stopping=10,
    ...
    def _fit(self, prefetch, val_set=None):
        ...
        # Create sampler
        sampler = dgl_utils.HearBlockSampler(self.node_review_graph, self.review_graphs, self.review_aggregator, self.dst_ntypes, self.ntype_ranges,
                                             fanout=self.fanout)
        ...
        for e in range(self.num_epochs):
            ...
            with tqdm(dataloader, disable=not self.verbose) as progress:
                for i, batch in enumerate(progress, 1):
                    ...
                    t = torch.zeros((edge_subgraph.num_nodes(), x['user'].shape[-1])).to(edge_subgraph.device)
                    for nt, emb in x.items():
                        t[self.node_filter(nt, edge_subgraph.ndata[dgl.NID])] = emb
                    x = t
    ...
    def _validate(self, val_set, metrics):
        ...
        self.model.eval()
        with torch.no_grad():
            self.model.inference(self.review_graphs, self.node_review_graph, self.device, self.node_filter)
            if self.objective == 'ranking':
