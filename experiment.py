...
def run(in_kwargs, dataset, method, save_dir='.'):
    ...
    if method in ['hear', 'testrec'] or method.startswith('hypar'):
        default_kwargs = {
            'use_cuda': True,
            'use_uva': False,
            'batch_size': 256,
    ...
    if dataset == 'cellphone':
        ...
        feedback = amazon_book_seer.load_feedback(fmt="UIRT", reader=Reader(min_user_freq=3))
        reviews = amazon_book_seer.load_review()
        sentiment = amazon_book_seer.load_sentiment(reader=Reader())
    else:
